...
public class TeleopSwerve extends CommandBase {
    ...
    public void execute() {
        ...
        double translationVal = translationLimiter.calculate(
                MathUtil.applyDeadband(translationSup.getAsDouble(), ConstantsSwerve.Swerve.stickDeadband));
        double strafeVal = strafeLimiter.calculate(
                MathUtil.applyDeadband(strafeSup.getAsDouble(), ConstantsSwerve.Swerve.stickDeadband));
        double rotationVal = rotationLimiter.calculate(
                MathUtil.applyDeadband(rotationSup.getAsDouble(), ConstantsSwerve.Swerve.stickDeadband));

        s_Swerve.drive(
                new Translation2d(translationVal, strafeVal).times(ConstantsSwerve.Swerve.maxSpeed),
                rotationVal * ConstantsSwerve.Swerve.maxAngularVelocity,
                isFieldOriented,
                true);
    }
}
